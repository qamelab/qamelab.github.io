...
**Design:** Swiss minimalist. Hanken Grotesk for UI, JetBrains Mono for
metadata (paper IDs, dates, tech stacks, formats). Burnt-orange accent
(`#c4622a`), generous whitespace, typography-driven hierarchy. Light + dark
mode — tokens defined in `:root` with `[data-theme="dark"]` overrides in
`global.css`; theme is chosen by the inline no-flash script in `Base.astro`
from `localStorage.theme` → `prefers-color-scheme` → light. Toggle lives in
the Header. The only decorative element is the short orange
`<hr class="rule">` below section headings.
...
- **Content collections** in `src/content/` (papers, publications, team, projects,
  ...
  `publications.fromQamePaper` uses Astro's `reference('papers')` — broken
  references fail `astro check` / the build.
...
- **Components** in `src/components/` — all `.astro`, no framework (React/Vue/etc).
  Reusable primitives: `PageHero` (section/page intros), `LogoCard` (software &
  data logo tiles), `AuthorList` (QAME-bold author rendering), `PaperCard`,
  `FeedSidebar`, `ResearchAreas`, `ProjectsGrants`.
...
- **Site metadata** (URL, contact email, social handles, affiliation) is
  centralized in `src/config/site.ts`. Import `site` from there rather than
  hardcoding strings. The Bluesky/Mastodon handles are duplicated at the top
  of `scripts/fetch-social.mjs` (the fetcher is plain Node ESM and can't
  import the TS config) — keep the two in sync.
- **Static assets** in `public/` (served at site root); lab logos and
  wordmarks live in `public/logos/` (light/dark variants). Build emits to
  `dist/`; hashed assets go under `dist/_assets/` per `astro.config.mjs`.
- **Sitemap** is auto-generated by `@astrojs/sitemap` at build time; no
  manual sitemap file.

## Current State

Homepage (`src/pages/index.astro`) is a full lab landing with Header/Footer,
a hero + feed sidebar, `ResearchAreas`, recent QAME Papers, and
`ProjectsGrants`. Content collections are populated with real data
(publications, projects, team, software); `grants/` is empty by design —
it is reserved for completed grants, and all current funded work lives in
`projects/`.
...
- Home page sidebar shows 3 most recent.
- Files are auto-generated by `scripts/fetch-social.mjs`, run on a 6-hour
  cron via `.github/workflows/fetch-social.yml` (commits changes back to
  `main` as `qame-news-bot`). Do not hand-edit JSON files — they will be
  overwritten on the next fetch. If both platforms return zero posts
  (outage, missing account), existing files are left intact so the build
  keeps working.
...
npm run check         # astro check — validates TS + content collection schemas
npm run build         # Build to dist/ (runs astro check implicitly via the deploy workflow)
npm run preview       # Preview production build
npm run fetch:social  # Refresh src/content/news/*.json from Bluesky + Mastodon
```

Run `npm run check` before pushing content changes — a typo in a required
frontmatter field or a broken `fromQamePaper` reference will fail the
deploy workflow the same way.
